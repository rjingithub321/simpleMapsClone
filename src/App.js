...
class App extends Component {
  ...
  handleSubmit = (event) => {
    ...
    const url = `https://api.greedy81.hasura-app.io/api/address?origin=${this.state.originadd}&destination=${this.state.destadd}`;
    fetch(url)
  ...
  handleSubmit2 = (event) => {
    event.preventDefault();
    const url = `https://api.greedy81.hasura-app.io/api/coordinates?lat=${this.state.lati}&long=${this.state.longi}`;
    fetch(url)
      .then(response => response.json())
